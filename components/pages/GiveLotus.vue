...
function validateOutAddress() {
  if (WalletTools.isValidAddress(outAddress.value)) {
    inputValidated.value.outAddress = true
    return 'success'
  }
  inputValidated.value.outAddress = false
}
/** Input field validation enforcing balance and decimal units */
function validateOutValue() {
  ...
  const balanceInt = toLotusUnits(props.balance)
  if (outValueInt > 0 && outValueInt <= balanceInt) {
    inputValidated.value.outValue = true
    return 'success'
  }
  ...
  <div class="py-2 px-6">
    ...
    <div class="py-2 flex">
      <div class="flex-grow">
        <FwbInput
          v-model="outAddress"
          :disabled="sending"
          :validation-status="validateOutAddress()"
    ...
    <div class="py-2">
      <FwbInput
        v-model="outValue"
        :disabled="sending"
        :validation-status="validateOutValue()"
        label="Amount of Lotus to Give"
        type="number"
      >
        <template #suffix class="right-0">
          <FwbButton disabled color="pink" :outline="true" size="sm"
            >Give All</FwbButton
          >
        </template>
      </FwbInput>
          ...
          <div>
            ...
            <span>
              (<a
                ...
                :href="DEFAULT_EXPLORER_URL + '/tx/' + sendLotusResult.txid"
